...
def plot_heatmap(experiment_data):
    ...
    # x as a multiplier
    x = list(range(1, 11))
    fig = sns.heatmap(z_2d_array, xticklabels=x, yticklabels=sorted(y, reverse=True), annot=True, fmt='d', cmap="Greens")
    fig.set_xlabel('Sample size multiplier relative to L*')
    fig.set_ylabel('Average query length')
    fig.set_title('Bluetooth CC2640R2-no-feature-req')
    # plt.show()
